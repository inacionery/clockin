...
public class PageableParameterBuilderPlugin implements ParameterBuilderPlugin {
    ...
    @Autowired
    public PageableParameterBuilderPlugin(TypeNameExtractor nameExtractor,
    ...
    public void apply(ParameterContext context) {
        ...
        if (type != null && Pageable.class.isAssignableFrom(type)) {
            ...
            ModelReference intModel = factory
                .apply(resolver.resolve(Integer.TYPE));
            ModelReference stringModel = factory
                .apply(resolver.resolve(List.class, String.class));

            List<Parameter> parameters = newArrayList(
                context.parameterBuilder().parameterType("query").name("page")
                    .modelRef(intModel)
                    .description("Page number of the requested page").build(),
                context.parameterBuilder().parameterType("query").name("size")
                    .modelRef(intModel).description("Size of a page").build(),
                context.parameterBuilder().parameterType("query").name("sort")
            ...
            context.getOperationContext().operationBuilder()
